...
5. Verifies the real audio track and format, names the file as `[uploader] video title`
   when YouTube metadata is available, and invokes the configured FunASR Python.
6. Writes `~/transcription_jobs/youtube-VIDEO_ID/txt/[uploader] video-title.txt`
   (falling back to just the video title if uploader metadata is unavailable).
